"""
=====================================================================================
  🍄  FACTORY ENERGY & COST MANAGEMENT DASHBOARD (Multi-Month & Excel Sync)
  ...
  Dark, monitoring-grade UI (Grafana-inspired) with Thai / English language toggle.
=====================================================================================
...
# =====================================================================================
# GLOBAL CONSTANTS — Grafana-inspired dark palette
# =====================================================================================
BG_APP = "#0B0D10"
BG_PANEL = "#181B1F"
BG_PANEL_ALT = "#1F2328"
...
LANG = {
    "en": {
        ...
        "settings_caption": (
            "Configure electricity rates, billing days, and factory"
            " departments."
        ),
        ...
        "section_dept_mgmt": "Department Management",
        "add_dept_label": "➕ Add New Department",
        "add_dept_input_label": "New Department Name",
        "add_dept_placeholder": "e.g. Cold Storage Warehouse",
        ...
        "success_dept_added": "Department '{name}' added.",
        "delete_dept_label": "🗑️ Delete Department",
        "delete_dept_select": "Select Department to Delete",
        "delete_dept_button": "Delete Department",
        "success_dept_deleted": "Department '{name}' deleted.",
        "info_no_departments": "No departments available.",
        "expander_dept_overview": "Current Departments Overview",
        "dept_machine_count": "{dept} — {n} machine type(s)",
    },
    "th": {
        ...
        "settings_caption": (
            "ตั้งค่าอัตราค่าไฟฟ้า จำนวนวันในรอบบิล และจัดการแผนกของโรงงาน"
        ),
...
# =====================================================================================
# CUSTOM CSS — dark, Grafana-inspired, minimal & professional
# =====================================================================================
def inject_css():
    st.markdown(
        f"""
...
# =====================================================================================
# SESSION STATE INITIALIZATION (Multi-Month & Machines)
# =====================================================================================
def init_session_state():
    ...
    # Default Multi-Month History (Supports 10+ months)
    ...
    if "departments" not in st.session_state:
        st.session_state.departments = {
            "Cooling System": pd.DataFrame([
                {
                    "Machine Name": "Industrial Chiller Unit",
                    "Quantity": 2,
                    "kW": 75.0,
                    "Peak Hours/day": 9.0,
                    "Off-Peak Hours/day": 15.0,
                },
                {
                    "Machine Name": "Cold Room Evaporator Fan",
                    "Quantity": 6,
                    "kW": 3.5,
                    "Peak Hours/day": 10.0,
                    "Off-Peak Hours/day": 14.0,
                },
            ]),
            "Autoclave & Sterilization": pd.DataFrame([
                {
                    "Machine Name": "Autoclave Sterilizer",
                    "Quantity": 3,
                    "kW": 45.0,
                    "Peak Hours/day": 6.0,
                    "Off-Peak Hours/day": 2.0,
                },
                {
                    "Machine Name": "Steam Boiler Feed Pump",
                    "Quantity": 2,
                    "kW": 7.5,
                    "Peak Hours/day": 6.0,
                    "Off-Peak Hours/day": 2.0,
            ...
            "Packing Line": pd.DataFrame([
                {
                    "Machine Name": "Conveyor Motor",
                    "Quantity": 4,
                    "kW": 2.2,
                    "Peak Hours/day": 8.0,
                    "Off-Peak Hours/day": 0.0,
                },
                {
                    "Machine Name": "Vacuum Sealing Machine",
                    "Quantity": 5,
                    "kW": 1.8,
                    "Peak Hours/day": 8.0,
                    "Off-Peak Hours/day": 0.0,
            ...
            "Utilities & Lighting": pd.DataFrame([
                {
                    "Machine Name": "Factory LED Lighting",
                    "Quantity": 120,
                    "kW": 0.04,
                    "Peak Hours/day": 10.0,
                    "Off-Peak Hours/day": 12.0,
                },
                {
                    "Machine Name": "Air Compressor",
                    "Quantity": 2,
                    "kW": 15.0,
                    "Peak Hours/day": 8.0,
                    "Off-Peak Hours/day": 4.0,
                },
            ]),
        }
...
def create_excel_template() -> bytes:
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if all_machines:
            ...
            combined_machines = pd.DataFrame(columns=MACHINE_COLUMNS)
        ...
        if "Machines" in xls.sheet_names:
            ...
            if all(col in mach_df.columns for col in required_mach_cols):
                ...
                for dept, group in mach_df.groupby("Department"):
                    ...
                    new_depts[str(dept)] = clean_group
                st.session_state.departments = new_depts
        return True
...
# CALCULATION ENGINE
# =====================================================================================
def compute_department_theoretical_kwh(
    departments: dict, billing_days: int
) -> pd.DataFrame:
    rows = []
...
def run_calculation_for_month(row_data):
    ...
    dept_theo = compute_department_theoretical_kwh(
        st.session_state.departments, billing_days
    )
...
def page_dashboard():
    ...
    if history_df.empty:
        st.warning("No monthly history data available. Please add data in Data Entry.")
        return
    ...
    if sel_idx > 0:
        prev_row = history_df[
            history_df["Month"] == months_list[sel_idx - 1]
        ].iloc[0]
    ...
    pct_vs_last = (
        (delta_vs_last / last_month_cost * 100) if last_month_cost else 0
    )

    # Calculate multi-month average (e.g., past 6 months or all available)
    all_costs = []
    ...
    with c3:
        kpi_card(
            t("kpi_vs_avg"), fmt_thb(six_avg), delta_vs_avg, pct_vs_avg
        )
    ...
    with s4:
        chip_metric(
            t("chip_theoretical"), fmt_kwh(dept_theo["Total kWh"].sum())
        )
    ...
    with col_chart:
        ...
        fig_alloc = go.Figure(
            go.Bar(
                ...
                text=[
                    fmt_thb(v) for v in alloc_sorted["Allocated Cost (THB)"]
                ],
...
def page_data_entry():
    ...
    # Excel Template Download & Upload Section
    st.markdown(
    ...
    with col_ul:
        uploaded_file = st.file_uploader(
            t("upload_excel_label"), type=["xlsx"]
        )
    ...
    # Section A: Multi-Month History Editor
    st.markdown(
    ...
    # Section B: Machine Working Hours by Department
    st.markdown(
...
def page_settings():
    ...
    with dcol1:
        ...
        if st.button(
            t("add_dept_button"), type="primary", use_container_width=True
        ):
    ...
    st.write("")
    with st.expander(t("expander_dept_overview")):
        for dept, df in st.session_state.departments.items():
            st.write(t("dept_machine_count", dept=dept, n=len(df)))
...
def render_sidebar():
    with st.sidebar:
        ...
        current_label = (
            "English" if st.session_state.lang == "en" else "ไทย"
        )
        chosen = st.selectbox(
            "🌐 " + t("language"),
        ...
        st.caption(
            f"{t('departments_tracked')}: **{len(st.session_state.departments)}**"
        )
        st.markdown("---")
        st.caption(t("footer"))
